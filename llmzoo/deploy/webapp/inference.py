    ...
    for i in range(max_new_tokens):
        if i == 0:
            if model.config.is_encoder_decoder:
                ...
                print(model)
                out = model(input_ids=torch.as_tensor([input_ids], device=device, dtype=torch.get_default_dtype()), use_cache=True)
                logits = out.logits
                past_key_values = out.past_key_values
        else:
            if model.config.is_encoder_decoder:
                ...
                out = model(
                    input_ids=torch.as_tensor([[token]], device=device, dtype=torch.get_default_dtype()),
                    use_cache=True,
                    past_key_values=past_key_values,
                )
                logits = out.logits
